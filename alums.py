...
# Navigating to each of the companies' webpages to find past alumni
used = set()
alums_links = [[] for _ in range(len(company_links))]
for i, link in enumerate(company_links):
    ...
    try:
        # If both these links exist, then there are alumnis at this company 
        alums = driver.find_element(By.XPATH, f"//div[@class='mt1']/div/a[1]")
        emps = driver.find_element(By.XPATH, f"//div[@class='mt1']/div/a[2]")
    except:
        continue
    ...
    for k in range(1, len(persons) + 1):
        ...
        for c, char in enumerate(profile_path):
            if char == '?':
                ...
                break
        alums_links[i].append(profile_path)

alumnis = []
for array in alums_links:
    temp = ','.join(array)
    alumnis.append(temp)

# Create pandas dataframe
df = pd.DataFrame(list(zip(company_links, job_descriptions, alumnis)),
       columns = ['company_links', 'descriptions', 'alumnis'])
